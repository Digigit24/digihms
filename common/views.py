...
class SuperAdminLoginView(LoginView):
    ...
    def form_valid(self, form):
        ...
        if user is not None:
            ...
            messages.error(
                self.request,
                'Invalid credentials or HMS module not enabled for your account'
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'title': 'DigiHMS Admin Login',
            'site_title': 'DigiHMS Administration',
            'site_header': 'DigiHMS Admin',
        ...
        try:
            ...
            try:
                ...
                if module_name not in enabled_modules:
                    return JsonResponse({
                        'error': f'{module_name} module not enabled for this user'
                    }, status=403)
        ...
        if response.status_code == 200:
            ...
            if access_token:
                ...
                try:
                    ...
                    if module_name not in enabled_modules:
                        logger.warning(f"{module_name} module not enabled for user")
                        return JsonResponse({
                            'error': f'{module_name} module not enabled for this user'
                        }, status=403)
            ...
            logger.warning(f"HMS module not enabled for {email}")
            return JsonResponse({
                'error': 'HMS module not enabled for your account.'
            }, status=403)
        else:
